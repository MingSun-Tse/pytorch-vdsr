...
v2: Add local structure training
"""
...
parser.add_argument("--pixel_threshold", type=float, default=10./255)
...
def test(model, epoch):
  ...
  for scale in scales:
    ...
    logprint("Epoch {} Scale {} PSNR_predicted = {:.4f} PSNR_bicubic = {:.4f}".format(epoch, scale, avg_psnr_predicted/count, avg_psnr_bicubic/count))
...
def train(training_data_loader, optimizer, model, loss_func, epoch):
    ...
    for step, batch in enumerate(training_data_loader, 1):
        ...
        if step % SHOW_INTERVAL == 0:
          global t1          
          # format_str = "E{}S{} loss={:.3f} | iloss={:.3f} | {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} \
# {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} \
# {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} \
# {:.3f} {:.3f} {:.3f} {:.3f} ({:.2f}s/step)"
          # logprint(format_str.format(epoch, step, loss.data.cpu().numpy(), iloss.data.cpu().numpy(), \
          # ploss_0, ploss_1, ploss_2, ploss_3, ploss_4, ploss_5, ploss_6, ploss_7, ploss_8, ploss_9, \
          # ploss_10, ploss_11, ploss_12, ploss_13, ploss_14, ploss_15, ploss_16, ploss_17, ploss_18, \
          # (time.time()-t1)/SHOW_INTERVAL))
          ...
          # logprint("{:.3f}".format((time.time() - t1)/SHOW_INTERVAL)) # test speed
          t1 = time.time()
        if step % 100 == 0:
          test(model, epoch)
